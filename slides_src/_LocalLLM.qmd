...
We only care about using Local LLM model that can provide [OpenAI compatible RESTful API](https://bentoml.com/llm/llm-inference-basics/openai-compatible-api){target="_blank"} endpoint, so that AutoGen for .NET agent can use it as LLM backend service.

## xPU TOPs myth

![](./pics/Intel_total_TOPs.png){.r-stretch}

Currrently most of **_NPU_(s)** are only support ONNX model format, and lack of some advanced features(ex: function-calling) that Agentic AI usually need.
...
::: columns
::: {.column width="50%"}
![External Thunderbolt GPU enclosure](./pics/Nvidia_gpt-oss-20b.png)
:::

::: {.column width="50%"}
![Built-in AMD GPU](./pics/AMD_gpt-oss-20b.png)
:::
:::
